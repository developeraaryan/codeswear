      ...
      <section className="text-gray-600 body-font">
        <div className="container px-5 py-24 mx-auto">
          <div className="flex flex-wrap -m-4 justify-center">
            {Object.keys(products).length === 0 && <p>Sorry, All the Mugs are out of stock , Please stay tuned! </p>}
...
export async function getServerSideProps(context) {
  ...
  for (let items of products) {
    if (items.title in Mugs) {
      if (!Mugs[items.title].color.includes(items.color) && items.availableqty > 0) {
        Mugs[items.title].color.push(items.color)
      }
      if (!Mugs[items.title].size.includes(items.size) && items.availableqty > 0) {
      ...
      if (items.availableqty > 0) {
        Mugs[items.title].color = [items.color]
