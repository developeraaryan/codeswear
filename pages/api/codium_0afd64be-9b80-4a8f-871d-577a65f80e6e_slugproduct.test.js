...
describe('handler', () => {
    ...
    // Tests that a non-GET request returns a 400 status code
    it('should return a 400 status code when a non-GET request is made', async () => {
        const req = { method: 'POST' };
        const res = { status: jest.fn().mockReturnThis(), json: jest.fn() };
        await handler(req, res);
        expect(res.status).toHaveBeenCalledWith(400);
        expect(res.json).toHaveBeenCalledWith({ message: 'We only accept GET requests' });
    });
});
